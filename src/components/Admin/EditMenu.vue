...
<button @click="addMenu" type="submit" class="btn btn-outline-dark mx-2 d-inline-block mt-3">Accept</button>
<button @click="goback" type="submit" class="btn btn-outline-dark d-inline-block mt-3">Back</button>
</template>
...
export default {
    setup(){
        ...
        const {  error, document } = getDocument('rooms', route.params.id)
        // const form = ref({
        //     category: '',
        //     price:'',
        //     peopleCount:'',
        ...
        //     service:[]
        // })
        const form = ref({...document.value})
        console.log(document.value)

        const goback = () => router.push({name:'menudata'})
        
        return { image, form , goback, error }
    }
}
</script>
<style scoped>
